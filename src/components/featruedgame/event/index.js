...
export default class EventPrice extends PureComponent {
    constructor(props) {
      super(props)
      this.state = {
    ...
    selectEvent(is_blocked,game,sport,region,competition, evtmarket, evnt){
      if(!is_blocked)
    ...
    render() {
      const {sport,region,competition, evnt, game, evtmarket, betSelections, oddType }= this.props
      return (
        <div ref={(el) => { this.priceBtn = el }} onClick={(e) =>  {e.stopPropagation();this.selectEvent(game.is_blocked,sport,region,competition,game, evtmarket, evnt)}  } className={`event ${evnt.name && evnt.name.toLowerCase()} ${betSelections[game.id] && betSelections[game.id].eventId == evnt.id ? 'selected-event' : ''} ${evnt.initialPrice ? (evnt.initialPrice > evnt.price) ? 'coeficiente-change-down' : (evnt.initialPrice < evnt.price) ? 'coeficiente-change-up' : '' : ''}`}>
